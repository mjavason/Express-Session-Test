...
class Controller {
  ...
  async authorizeUserFromMagicLink(req: Request, res: Response) {
    ...
    if (!decoded)
      return AccessTokenErrorResponse(res, 'Unauthorized! Invalid token');
  ...
  async resetPasswordMail(req: Request, res: Response) {
    ...
    if (!mailSent)
      return InternalErrorResponse(res, 'Error sending password reset email');
  ...
  async resetPassword(req: Request, res: Response) {
    ...
    if (!updatedUser)
      return InternalErrorResponse(res, 'Unable to update password');

    // Delete the used reset token
    const usedToken = await resetTokenService.softDelete({
      _id: resetToken._id,
    });
